...
rsol.bttf = (function() {
	 ...
	 */
	var loaded = function () {
		var arrivalDate = destinationTime();
		var todayDate = presentTime();
		timeLeft(arrivalDate, todayDate);
		lastTimeDeparted();
		response(todayDate);
	...
	/**
	 * Calculates the time left until Marty McFly travels to the future and
	 * displays that time in a readable format.
	 */
	var timeLeft = function (destinationTime, presentTime) {
		...
		if (destinationTime.fromNow(true) < '30 years' || destinationTime.fromNow(true) > '12 months') {
	...
	/**
	 * Displays the response text if today's date is before the destination time.
	 */
	var response = function (todayDate) {
		if (todayDate.format('YYYY-MM-DD') < moment('2015-10-21').format('YYYY-MM-DD')) {
			$('#response').text('NO');
		} else if (todayDate.format('YYYY-MM-DD') > moment('2015-10-21').format('YYYY-MM-DD')) {
			$('#response').text('Well..');
			$('#arrival').text('Marty McFly already visited us.');
		} else {
			$('#response').text('YES');
			$('#arrival').text('Marty McFly should be here today!');
		}
	};
